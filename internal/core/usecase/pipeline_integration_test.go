...
func (s *capturingSink) Notify(_ context.Context, f anomaly.Finding) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.fs = append(s.fs, f)
	return nil
}
func (s *capturingSink) Findings() []anomaly.Finding {
	s.mu.Lock()
	defer s.mu.Unlock()
	out := make([]anomaly.Finding, len(s.fs))
	copy(out, s.fs)
	return out
}
...
func TestPipelineEndToEndAnomalyFiresAndReachesTelegram(t *testing.T) {
	...
	tg, err := alerting.NewTelegramSink(alerting.TelegramConfig{
	...
	cap := &capturingSink{}
	fanout := &alerting.Fanout{Sinks: []alerting.Sink{cap, tg}, Logger: &log}

	det := detect.New(detect.Config{
		Interval:      time.Hour,
		RecentWindows: []time.Duration{time.Hour},
		Rule:          anomaly.Rule{Multipliers: []float64{30, 100, 1000}},
		Cooldown:      time.Hour,
		Clock:         clock,
	...
	for _, f := range findings {
		if f.Severity == anomaly.SeverityFatal {
			sawFatal = true
		}
	}
	if !sawFatal {
