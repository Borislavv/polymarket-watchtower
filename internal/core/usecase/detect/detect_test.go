...
func (e *capturingEmitter) all() []anomaly.Finding {
	e.mu.Lock()
	defer e.mu.Unlock()
	cp := make([]anomaly.Finding, len(e.findings))
	copy(cp, e.findings)
	return cp
}
...
func newDetectFixture(t *testing.T, now time.Time, eng *aggregate.Engine, mult []float64, minNotional float64, minTrades int) (*Loop, *aggregate.MarketRegistry, *capturingEmitter) {
	...
	loop := New(Config{
		...
		Rule:          anomaly.Rule{Multipliers: mult, MinNotional: minNotional, MinTrades: minTrades},
		Cooldown:      time.Hour,
		Clock:         func() time.Time { return now },
...
func TestDetectFiresOnX30Spike(t *testing.T) {
	...
	for _, f := range findings {
		if f.Severity == anomaly.SeverityWarn {
			sawWarn = true
		}
...
func TestDetectVolumeFloorBlocksFire(t *testing.T) {
	...
	loop.Tick(context.Background())
	for _, f := range emit.all() {
		if f.Metric != anomaly.MetricAvgSize {
