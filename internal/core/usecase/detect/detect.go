...
import (
	...
	"github.com/Borislavv/polymarket-watchtower/internal/core/domain/trade"
...
func (l *Loop) tick(ctx context.Context) {
	...
	for _, m := range snapshot {
		...
		for _, w := range l.cfg.RecentWindows {
			...
			l.evaluate(ctx, now, anomaly.ScopeMarket, string(m.ID), m.Slug,
				anomaly.MetricTradeRate, recent.TradesPerMinute(), baseline.TradesPerMinute(),
				recent, w, m.ID, 0)
			l.evaluate(ctx, now, anomaly.ScopeMarket, string(m.ID), m.Slug,
				anomaly.MetricNotionalRate, recent.NotionalPerMinute(), baseline.NotionalPerMinute(),
				recent, w, m.ID, 0)
			l.evaluate(ctx, now, anomaly.ScopeMarket, string(m.ID), m.Slug,
				anomaly.MetricAvgSize, recent.AvgSize(), baseline.AvgSize(),
				recent, w, m.ID, 0)
		}
	}
...
func (l *Loop) evaluateCategories(ctx context.Context, now time.Time, snapshot []market.Market) {
	...
	for cat, acc := range cats {
		for _, w := range l.cfg.RecentWindows {
			...
			l.evaluate(ctx, now, anomaly.ScopeCategory, target, acc.label,
				anomaly.MetricTradeRate, recentTPM, baseTPM, r, w, "", cat)
			l.evaluate(ctx, now, anomaly.ScopeCategory, target, acc.label,
				anomaly.MetricNotionalRate, recentNPM, baseNPM, r, w, "", cat)
		}
	}
}
...
func (l *Loop) evaluate(
	...
	if metric != anomaly.MetricAvgSize {
...
// don't emit a link.
func marketURL(scope anomaly.Scope, slug string) string {
	if scope != anomaly.ScopeMarket || slug == "" {
		return ""
	}
